...
BoardCost is a single-page web app (HTML, CSS and vanilla JavaScript) that turns a display specification — type, dimensions, board grade, print process, finishing and quantity — into a full, line-by-line cost estimate with quantity price breaks and CSV export. It was built as the software engineering project for my Digital Technology Solutions degree apprenticeship, and this README is the principal report for that assignment. Each numbered section below maps directly to one of the ten assignment subtasks.
...
I work in IT at a company in the point-of-sale cardboard display industry. We design and manufacture corrugated retail displays — free-standing display units (FSDUs), counter display units (CDUs), standees and dump bins — for brands who want their products standing out in stores.
...
**Why this scope suits the assignment.** It is a real problem from my employer's domain, small enough to design, plan, test-drive and document properly within the assignment window, but with genuine business logic (a costing model with setup amortisation, waste factors and discount tiers) rather than a trivial form. It fits option (a) from the brief: a simple web app written in HTML, CSS and JavaScript.
...
I designed the interface before writing any UI code, working from low fidelity to high fidelity (I prototyped with quick drawn wireframes rather than Figma, which the brief allows as "Figma or an alternative" — the artefacts are exported to `docs/design/`).
...
I planned the project as three one-week agile sprints, managed on a **GitHub Projects** kanban board attached to this repository (Projects is the free GitHub-native alternative to Jira suggested by the brief). The board has four columns — **Backlog → Sprint (To do) → In progress → Done** — and every card on it is a real issue from this repo, so the board, the issues and the commit history all describe the same work.
...
**CI/CD.** `.github/workflows/ci.yml` runs two jobs. **test** runs on every push and pull request to `main`: checkout → Node 20 (with npm caching) → `npm ci` → `npm test -- --ci`. Because the repo's `main` branch expects green CI, a broken suite blocks a merge — success criterion S3. **deploy** runs only on pushes to `main`, only after **test** succeeds, and publishes the static site to GitHub Pages. That gives a genuinely useful CD property for a tool like this: the URL account managers would bookmark can only ever serve code that has passed the full suite.

## 7. Adding features gradually with Git and GitHub

The repository history *is* the build narrative from section 5. The workflow was one ticket → one branch → one pull request → merge to `main`, with branch names and commit messages carrying the ticket number:
...
Two things I'd point a reviewer at in the log: the engine branch shows TDD in the commit sequence itself (`test: add failing tests…` followed by `feat: … tests green`), and the bugfix branch shows the same discipline applied to a defect (the failing test is a separate commit *before* the fix). Merges into `main` were non-fast-forward so each feature remains a visible, revertable unit in the graph — `git log --graph --oneline` reads like the sprint plan. Documentation commits went straight to `main` at the end, which is a pragmatic exception I'd defend: docs-only changes can't break CI-tested code.
...
- **Technical documentation — [`docs/TECHNICAL.md`](docs/TECHNICAL.md).** The architecture (and *why* the engine/UI split exists), the cost model step by step, how to run the app locally (no build step — open `index.html` or `npx http-server .`), how to run the tests (`npm install`, `npm test`, plus watch and coverage modes), the CI/CD pipeline, and the contributing workflow.
